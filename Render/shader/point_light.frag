...
void main() {
    ...
    vec3 L = normalize(vPosition - pos);
    vec3 H = normalize(V + L);
    float dist = length(vPosition - pos);
    float attenuation = 1.0 / (vAttenuation.x + vAttenuation.y * dist + vAttenuation.z * (dist * dist));
