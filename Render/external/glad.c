...
static int get_exts(void) {
    ...
    if(max_loaded_major < 3) {
        ...
        for(_index = 0; _index < num_exts_i; _index++) {
            const char *gl_str_tmp = (const char*)glGetStringi(GL_EXTENSIONS, _index);
...
static int has_ext(const char *ext) {
    ...
    if(max_loaded_major < 3) {
        ...
        for(_index = 0; _index < num_exts_i; _index++) {
            const char *e = exts_i[_index];

            if(exts_i[_index] != NULL && strcmp(e, ext) == 0) {
                return 1;
            }
        }
    }
